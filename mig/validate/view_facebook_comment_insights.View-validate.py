...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_ai' AND table_name = 'view_facebook_comment_insights'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_ai.view_facebook_comment_insights"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
tablename = databricksCatalog + "." + databricksSchema + "." + "view_facebook_comment_insights"
