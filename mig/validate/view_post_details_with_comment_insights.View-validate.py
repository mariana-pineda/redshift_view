...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_ai' AND table_name = 'view_post_details_with_comment_insights'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_ai.view_post_details_with_comment_insights"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
tablename = databricksCatalog + "." + databricksSchema + "." + "view_post_details_with_comment_insights"
...
# Check each column
charColValidate = """select sum((COLNAME IS NULL)::int) as null, round(avg(cast(len(rtrim(COLNAME)) as float)), 2) as avg, min(len(rtrim(COLNAME))) as min, max(len(rtrim(COLNAME))) as max, round(stddev(len(rtrim(COLNAME))), 2) as stddev from """
intColValidate = """select sum((COLNAME IS NULL)::int) as null, round(avg(cast(COLNAME as float)), 2) as avg, min(COLNAME) as min, max(COLNAME) as max, round(stddev(COLNAME), 2) as stddev from """
redshiftColValidate = charColValidate + redshiftTablename
databricksColValidate = charColValidate + tablename
redshiftIntValidate = intColValidate + redshiftTablename
databricksIntValidate = intColValidate + tablename
