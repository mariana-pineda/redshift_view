...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_ai' AND table_name = 'events'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_ai.events"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
tablename = databricksCatalog + "." + databricksSchema + "." + "events"
